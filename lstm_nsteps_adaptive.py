...
for pair in data:
    ...
    for ep in range(30):
        for i in range(windowSize,3000):
            model.fit(X[i-windowSize:i][:].reshape(1,windowSize,X.shape[1]),encoded_Y[i].reshape(1,1),epochs=1, batch_size=1,verbose=0) 
        for i in range(windowSize,3000):
            model.fit(X2[i-windowSize:i][:].reshape(1,windowSize,X2.shape[1]),encoded_Y2[i].reshape(1,1),epochs=1, batch_size=1,verbose=0) 
    ...
    for i in range(windowSize,3000):
        results.extend(model.predict(testX[i-windowSize:i][:].reshape(1,windowSize,testX.shape[1]))[0]) 
        tmp = 0
        if i >= windowSize*2-1 :
            for j in range(windowSize):
                #print(len(results), i-2*windowSize+j+1,len(encoded_testY))
                tmp += abs(results[i-2*windowSize+j+1] - encoded_testY[i-windowSize+j+1])

            #print(tmp)
            if tmp > num_false:
                model.fit(testX[i-windowSize:i][:].reshape(1,windowSize,testX.shape[1]), \
